...
const VideoProgressBar = ({videoUrl}: Props) => {
	...
	const defineProgress = () => (progress < 0.6 ? `${0.5}%` : `${progress}%`);

	const defineBorder = () => {
		if (hideVideoPreviewBorder) return "transparent";
		return colors?.baseColor ? colors.baseColor : "#0ea5e9";
	};
	...
	return (
		...
		>
			...
			<button
				...
				className={cn(
					"rounded-md z-50 bg-sky-500 border-none outline-none absolute top-0"
				)}
			/>
			<button
				style={{
				...
				className='absolute bg-neutral-500 rounded-md border-none outline-none'
			/>
			<div
				onMouseDown={handleProgressBarDragStart}
				style={{
